...
c---------------------------------------------------------------------
c FT benchmark - This Code only works for CLASS=S & CLASS=C since they're symmetric(equal in all three dimensions).
c---------------------------------------------------------------------
...
#ifdef __FFTW__
      include 'fftw3.f'
      integer*8 plan_x_z_forward,plan_x_z_backward
      integer*8 plan_y_forward, plan_y_backward
      integer rank,howmany,vector(1)
      integer istride,idist,inembed(3),
     >        ostride,odist,onembed(3)
      integer i,j,k,indi
...
c---------------------------------------------------------------------
     ...
     >                  x1, x1, scratch)
            ...
            call dfftw_plan_many_dft(plan_x_z_backward, rank, vector,   
     >            howmany, 
     >            x1, inembed, istride, idist, 
     >            x1, onembed, ostride, odist, 
     >            FFTW_BACKWARD, FFTW_ESTIMATE)
            call dfftw_execute_dft(plan_x_z_backward, x1, x1)
#endif
     ...
     >                  x1, x1, scratch)
            ...
            call dfftw_plan_many_dft(plan_y_backward, rank, vector,
     >            howmany,
     >            x1, inembed, istride, idist,
     >            x1, onembed, ostride, odist,
     >            FFTW_BACKWARD, FFTW_ESTIMATE)
           ...
           do k = 1, dims(3,2)
              indi = (k - 1) * dims(1,2) * dims(2,2) + 1
            call dfftw_execute_dft(plan_y_backward, x1(indi), x1(indi))
           end do

            call dfftw_destroy_plan(plan_y_backward)
     ...
     >                  x2, x2, scratch)
            ...
            call dfftw_execute_dft(plan_x_z_backward, x2, x2)
            call dfftw_destroy_plan(plan_x_z_backward)
#endif
         else if (layout_type .eq. layout_2d) then
            call cffts1(1, dims(1,1), dims(2,1), dims(3,1), 
     >                  x1, x1, scratch)
     ...
     >                  x1, x1, scratch)
           ...
           call dfftw_plan_many_dft(plan_x_z_forward, rank, vector,
     >           howmany,
     >           x1, inembed, istride, idist, 
     >           x1, onembed, ostride, odist,
     >           FFTW_FORWARD, FFTW_ESTIMATE)
           call dfftw_execute_dft(plan_x_z_forward, x1, x1)
     ...
     >                  x2, x2, scratch)
          ...
          call dfftw_plan_many_dft(plan_y_forward, rank, vector,
     >          howmany,
     >          x1, inembed, istride, idist,
     >          x1, onembed, ostride, odist,
     >          FFTW_FORWARD, FFTW_ESTIMATE)
         ...
         do k = 1, dims(3,2)
            indi = (k - 1) * dims(1,2) * dims(2,2) + 1
         call dfftw_execute_dft(plan_y_forward, x2(indi), x2(indi))
         end do

         call dfftw_destroy_plan(plan_y_forward)
#endif
     ...
     >                  x2, x2, scratch)
           ...
           call dfftw_execute_dft(plan_x_z_forward, x2, x2)
           call dfftw_destroy_plan(plan_x_z_forward)
